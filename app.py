...
diseases_list = {15: 'Fungal infection', 4: 'Allergy', 16: 'GERD', 9: 'Chronic cholestasis', 14: 'Drug Reaction', 33: 'Peptic ulcer disease', 1: 'AIDS', 12: 'Diabetes ', 17: 'Gastroenteritis', 6: 'Bronchial Asthma', 23: 'Hypertension ', 30: 'Migraine', 7: 'Cervical spondylosis',
...
def health_assist(dis):
    ...
    precautions_to_take = [
        col for col in precautions_to_take.values if pd.notna(col)].flatten()
    medications = medications_df[medications_df['Disease']
                                 == dis]['Medication']
    medications = [
        medications for medications in medications.values if pd.notna(medications)]
...
symptoms = st.text_input("Input Symptoms (e.g., itching, skin_rash):")
...
if st.button("Predict"):
    if symptoms:
        ...
        if predicted_disease == "Unknown Disease":
            st.error(
                "Some of the symptoms you entered are not recognized. Please enter valid symptoms.")
        else:
            ...
            if precautions_to_take:
                for precaution in precautions_to_take:
...
st.sidebar.info(
    "Welcome to SymptoFinder, your reliable health assistant for understanding and managing common medical conditions. SymptoFinder is designed to help users quickly identify potential illnesses based on their symptoms and receive personalized recommendations for care.\n\n"
    "SymptoFinder utilizes advanced machine learning algorithms to analyze the symptoms you input and match them with a comprehensive database of known medical conditions. Our system then provides a list of potential conditions along with detailed descriptions, precautions, and suggested treatments."
)
